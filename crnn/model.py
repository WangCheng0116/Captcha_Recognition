...
#         # Initial convolutional layers with residual blocks
#         self.conv1 = ResidualBlock(1, 128)
#         self.max_pool1 = nn.MaxPool2d(2)
#         self.conv2 = ResidualBlock(128, 64)
#         self.max_pool2 = nn.MaxPool2d(2)

#         # Fully connected layers
#         self.linear = nn.Linear(1280, 128)  # Increased feature size for better capacity
#         self.dropout = nn.Dropout(0.3)  # Slightly increased dropout rate for regularization

#         # Enhanced GRU module
#         self.gru = nn.GRU(128, 64, bidirectional=True, num_layers=3, dropout=0.3)  # Increased layers and hidden units

#         # Output layer
#         self.out = nn.Linear(128, num_classes + 1)  # Adjusted output to match GRU hidden size
...
#         x = self.conv2(x)
#         x = self.max_pool2(x)
#         x = x.permute(0, 3, 1, 2)  # Reordering for linear input
#         x = x.view(bs, x.size(1), -1)
#         x = self.linear(x)
#         x = self.dropout(x)
#         x, _ = self.gru(x)
#         x = self.out(x)
#         x = x.permute(1, 0, 2)  # Permutation for CTC loss
